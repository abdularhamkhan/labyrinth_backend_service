...
const updateRealTimeCounters = async (event: AnalyticsEvent): Promise<void> => {
  try {
    ...
    logger.error(
      "Failed to update real-time counters:",
      isError(error) ? error : new Error(String(error))
    );
  }
};
...
export const getUserEngagementMetrics = async (userId: string): Promise<UserEngagementMetrics> => {
  try {
    ...
    const metrics: UserEngagementMetrics = {
      ...
      projectsCreated: user.workspaces.reduce(
 ...
 */
export const getProjectProgressMetrics = async (
  projectId: string
...
const calculateAverageProjectCollaborators = async (): Promise<number> => {
  try {
    ...
    const totalCollaborators = projects.reduce(
      (sum, project) => sum + project._count.collaborators,
      0
    );
