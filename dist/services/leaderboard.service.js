...
exports.updateUserScore = exports.rebuildLeaderboard = exports.getUserRank = exports.getDailyLeaderboard = exports.getWeeklyLeaderboard = exports.getGlobalLeaderboard = void 0;
const prisma_1 = require("../config/prisma");
const cachingStrategies_1 = require("../redis/strategies/cachingStrategies");
...
// =============================================================================
const getWeeklyLeaderboard = async (query) => {
    ...
    try {
        ...
        const weeklyGameData = await prisma_1.prisma.gameSession.findMany({
            where: {
                status: "COMPLETED",
                endedAt: {
                    gte: weekStart,
        ...
        const formattedPlayers = sortedPlayers.map((player, index) => ({
            rank: query.offset + index + 1,
...
// =============================================================================
const getDailyLeaderboard = async (query) => {
    ...
    try {
        ...
        const dailyGameData = await prisma_1.prisma.gameSession.findMany({
            where: {
                status: "COMPLETED",
                endedAt: {
                    gte: today,
...
// USER RANK SERVICE
// =============================================================================
const getUserRank = async (userId, type) => {
    console.log(`🎯 Getting user rank: ${userId} (${type})`);
    try {
        const cacheKey = getCacheKey(type);
        // Try to get rank from Redis
        const rank = await cachingStrategies_1.LeaderboardCache.getUserRank(type, userId);
        ...
        if (type === "global") {
            ...
            totalPlayers = await prisma_1.prisma.user.count({
                where: {
                    status: "ACTIVE",
                    gamesPlayed: { gt: 0 },
                },
            ...
            const cacheScore = await redis_1.cacheRedis.zscore(cacheKey, userId);
            userScore = cacheScore ? parseInt(cacheScore.toString()) : 0;
            totalPlayers = await redis_1.cacheRedis.zcard(cacheKey);
        }
        const percentile = rank && totalPlayers > 0
...
// =============================================================================
const rebuildLeaderboard = async (type) => {
    console.log(`🔄 Rebuilding ${type} leaderboard`);
    const startTime = Date.now();
    try {
        // Clear existing cache
        const cacheKey = getCacheKey(type);
        await redis_1.cacheRedis.del(cacheKey);
        // Rebuild based on type
        let playersProcessed = 0;
        switch (type) {
            case "global":
                await cachingStrategies_1.LeaderboardCache.rebuildLeaderboard("global");
                playersProcessed = await prisma_1.prisma.user.count({
                    where: {
                        status: "ACTIVE",
                        gamesPlayed: { gt: 0 },
                    },
            ...
            case "weekly":
                ...
                const weeklyData = await (0, exports.getWeeklyLeaderboard)({ limit: 1000, offset: 0 });
                playersProcessed = weeklyData.totalPlayers;
                break;
            case "daily":
                // Rebuild daily leaderboard from game sessions
                const dailyData = await (0, exports.getDailyLeaderboard)({ limit: 1000, offset: 0 });
...
exports.rebuildLeaderboard = rebuildLeaderboard;
// =============================================================================
// UPDATE USER SCORE SERVICE
// =============================================================================
const updateUserScore = async (userId, scoreIncrement, gamesWon = 0, gamesPlayed = 1) => {
    ...
    try {
        ...
        const newWinRate = updatedUser.gamesPlayed > 0 ? updatedUser.gamesWon / updatedUser.gamesPlayed : 0;
        ...
        await Promise.all([
            cachingStrategies_1.LeaderboardCache.updateLeaderboard("global", userId, updatedUser.totalScore),
            // Weekly and daily leaderboards are updated when games are completed
        ]);
        // Invalidate user caches
        await cachingStrategies_1.CacheInvalidation.invalidateUserCaches(userId);
...
// HELPER FUNCTIONS
// =============================================================================
const getCachedLeaderboard = async (cacheKey, query) => {
    try {
        // Try to get paginated data from cache
        const cachedPlayers = await redis_1.cacheRedis.zrevrange(cacheKey, query.offset, query.offset + query.limit - 1, "WITHSCORES");
        if (cachedPlayers.length === 0) {
            return null;
        }
        // Get total count
        const totalPlayers = await redis_1.cacheRedis.zcard(cacheKey);
        // Format players data
        const players = [];
...
const cacheLeaderboard = async (cacheKey, players, totalCount) => {
    try {
        // Clear existing cache
        await redis_1.cacheRedis.del(cacheKey);
        // Add all players to sorted set
        if (players.length > 0) {
            const pipeline = redis_1.cacheRedis.pipeline();
