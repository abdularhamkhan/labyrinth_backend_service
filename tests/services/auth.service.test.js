...
jest.mock('../../src/constants/error', () => ({
    ...
    AuthenticationError: class extends Error {
        constructor(message, code, details) { super(message); }
    },
...
describe('Auth Service', () => {
    ...
    describe('signupService', () => {
        it('should return requiresVerification on successful signup', async () => {
            ...
            const result = await (0, auth_service_1.signupService)({
    ...
    describe('verifyOtpService', () => {
        it('should return user and token on successful OTP verification', async () => {
            ...
            const result = await (0, auth_service_1.verifyOtpService)({ email: 'test@example.com', otp: '123456' });
            expect(result).toEqual({
                id: '123',
                username: 'testuser',
                token: 'test-token',
        ...
        it('should throw AuthenticationError on invalid OTP', async () => {
            ...
            await expect((0, auth_service_1.verifyOtpService)({ email: 'test@example.com', otp: 'wrong-otp' })).rejects.toThrow('Invalid OTP');
        });
    });
    ...
    describe('loginService', () => {
        it('should return user and token on successful login', async () => {
            ...
            const result = await (0, auth_service_1.loginService)({ emailOrUsername: 'test@example.com', password: 'password123' });
            expect(result).toEqual({
                id: '123',
                username: 'testuser',
                token: 'test-token',
        ...
        it('should throw AuthenticationError on invalid credentials', async () => {
            ...
            await expect((0, auth_service_1.loginService)({ emailOrUsername: 'test@example.com', password: 'wrong-password' })).rejects.toThrow('Invalid credentials');
        });
    });
});
